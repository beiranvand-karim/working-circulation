...
$name = $json.name;
$WorkingDirectory = $json.WorkingDirectoryForStandalone;
Push-Location $WorkingDirectory;

$name | ForEach-Object { $_ } {
    $RefinedName = $_.TrimStart().TrimEnd().Replace(" ", "-")
    ng new $RefinedName --skip-tests --style=scss --ssr=false --routing --inline-style=false --inline-template=false;
}
