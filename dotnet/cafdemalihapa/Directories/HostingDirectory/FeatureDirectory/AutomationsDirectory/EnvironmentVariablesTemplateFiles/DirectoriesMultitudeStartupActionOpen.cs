...
{
    public class DirectoriesMultitudeStartupActionOpen(
        ...
        CodeBase codeBase,
        ILogger<DirectoriesMultitudeStartupActionOpen> logger
        )
    {
                ...
                if (environmentVariablesSourceDictionary.TryGetValue(key, out var val))
                {
                    try
                    {
                        switch (key)
                        {
                            case "COMMAND":
                                {
                                    var wrappedVal = stringHelpers.WrapInQuotationMarks("directory-to-be-open");
