...
$ArgumentList = "--application $env:APPLICATION --command $env:COMMAND --order $env:ORDER --notepaddpp-execute-file-location $env:NOTEPADDPP_EXECUTE_FILE_LOCATION --feature-name $env:FEATURE_NAME  --hosting-directory $env:HOSTING_DIRECTORY --primary-application-name $env:PRIMARY_APPLICATION_NAME --secondary-application-name $env:SECONDARY_APPLICATION_NAME"
