<#> fina = file name = ide-jetbrains-rider-multitude-tertiary-action-open.ps1 #>

get-content "ide-jetbrains-rider-multitude-tertiary-action-open.env" | ForEach-Object {
    $name, $value = $_.split("=")
...
if ($IsLinux) {
    start-process -FilePath $env:CAFDEM_EXECUTIVE_FILE_ADDRESS -Wait -RedirectStandardOutput "./output.txt" -ArgumentList $ArgumentList
