...
type IdeMicrosoftVscodeInsidersMultitudePrimaryActionOpen
    (
        featureName: FeatureName,
        secondaryApplication: SecondaryApplication,
        primaryApplication: PrimaryApplication,
        stringHelpers: StringHelpers,
        logger: ILogger<IdeMicrosoftVscodeInsidersMultitudePrimaryActionOpen>
    ) =
    member _.PairUpVariablesWithTheirValue(fileNamePath: string, environmentVariablesSourceDictionary: Dictionary<string, string>) =
        ...
        while not (isNull line) do
            ...
            with _ ->
                logger.LogError("IdeMicrosoftVscodeInsidersMultitudePrimaryActionOpen: the key could not be processed: {Key}", key)
